...
async function record(receipt){await mkdir(new URL('../verification/receipts/',import.meta.url),{recursive:true});await writeFile(new URL(`../verification/receipts/${receipt.hash}.json`,import.meta.url),encode(receipt));console.log(encode({hash:receipt.hash,status:receipt.status_name,result:receipt.result_name,execution:receipt.consensus_data?.leader_receipt?.[0]?.execution_result}));}
